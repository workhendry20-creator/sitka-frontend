    ...
    <div className="flex min-h-screen bg-slate-50 relative font-sans">
      ...
      <aside className={`fixed md:static inset-y-0 left-0 z-50 w-72 bg-[#0a1e36] text-white flex flex-col transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        <div className="p-8 flex items-center gap-3">
          ...
          <h1 className="text-2xl font-black tracking-tight">ADMIN SITKA</h1>
        </div>
        ...
        <nav className="flex-1 px-4 space-y-2 mt-4">
          {menuItems.map((item) => {
            ...
            return (
              <Link key={item.name} to={item.path} className={`flex items-center gap-4 px-4 py-4 rounded-2xl font-bold transition-all ${isActive ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/40' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}>
      ...
      <main className="flex-1 flex flex-col min-w-0">
        <header className="h-20 bg-white border-b border-gray-100 flex items-center justify-between px-8 sticky top-0 z-30">
          <button onClick={() => setIsSidebarOpen(true)} className="md:hidden text-[#0a1e36]"><Menu size={24} /></button>
          ...
          <div className="flex items-center gap-6">
            <button className="relative text-slate-400 p-2"><Bell size={22} /><span className="absolute top-2 right-2 w-2 h-2 bg-indigo-500 rounded-full border-2 border-white"></span></button>
            <div className="flex items-center gap-4 text-right group cursor-pointer">
              ...
              <div className="w-12 h-12 bg-indigo-100 rounded-2xl flex items-center justify-center font-black text-indigo-600 border-2 border-white shadow-sm">A</div>
            </div>
          </div>
        </header>
